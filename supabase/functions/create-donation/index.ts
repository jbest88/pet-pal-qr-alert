...
serve(async (req) => {
  ...
  try {
    ...
    const { amount = 2000 } = await req.json();
    logStep("Donation amount", { amount });
    ...
    logStep("Creating donation session", { amount });
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: { name: "PetAlert Donation" },
            unit_amount: amount,
          },
          quantity: 1,
        },
    ...
    logStep("Donation session created", { sessionId: session.id, url: session.url });
